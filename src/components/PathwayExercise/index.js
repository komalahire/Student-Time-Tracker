...
function PathwayExercise() {
  ...
  useEffect(() => {
    ...
    axios
      .get(`https://merd-api.merakilearn.org/courses/87/exercises        `)
      .then((response) => {
      ...
      <Container maxWidth="lg" margin="64px 0px">
        <Grid container spacing={2}>
          <Grid item xs={12} sm={12} md={10} lg={10}>
            {exercise?.course?.exercixses?.map((item) => {
              return (
                item.id === exerciseId &&
                item.content.map((content) => (
                  <>
                    {content.component === "header" && (
                      <Box sx={{ margin: "30px 0 10px 0", alignItems: "left" }}>
                        {headingVarients[content.variant](
              ...
              {exercise?.course &&
                exercise.course.exercises.map((item) => (
                  <ListItem key={item.id} component="div">
                    <ListItemButton onClick={() => setExerciseId(item.id)}>
